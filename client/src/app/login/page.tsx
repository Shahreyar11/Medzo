...
export default function Login() {
  ...
  // form is valid the moment both fields have something in them
  const formValid = user.email.length > 0 && user.password.length > 0;

  const onLogin = async () => {
    try {
      setSubmitting(true);

      // posting credentials to backend login route
      ...
      <div className="w-full max-w-md bg-white rounded-2xl border border-[#16423C]/10 shadow-[0_4px_30px_rgba(22,66,60,0.08)] p-8">
        ...
        <div className="flex flex-col items-center mb-7">
          ...
          <h1 className="mt-3 text-2xl font-[Space_Grotesk,sans-serif] font-bold text-[#16423C]">
            {submitting ? "Logging you in..." : "Welcome back"}
          </h1>
          <p className="text-[#6B7C78] text-sm mt-1">Log in to your Medzo account</p>
        </div>

        {/* Form */}
        <div className="flex flex-col gap-4">
          <div>
            <label htmlFor="email" className="text-sm font-semibold text-[#16423C] mb-1.5 block">
              Email
            </label>
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              <label htmlFor="password" className="text-sm font-semibold text-[#16423C]">
                Password
              </label>
              <Link href="/forgot-password" className="text-xs text-[#6B7C78] hover:text-[#16423C] hover:underline">
                Forgot password?
              </Link>
            </div>
            <div className="relative">
...
/* Same signature pulse-line mark used in the navbar and signup page */
function PulseMark() {
  return (
